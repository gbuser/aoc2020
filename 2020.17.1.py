...
universe =[[ [0] * zrange for y in range(yrange)] for x in range(xrange)]
...
for x in range(1, xrange - 1):
    for y in range(1, yrange - 1):
        for z in range(1, zrange - 1):
            universe_coords.append((x, y, z))

for x in range(xstart):
    for y in range(ystart):
        if data[x][y] == '#':
            universe[x + 7][y + 7][7] = 1
...
def generation(volume):
    ...
    for item in change_list:
        x, y, z = item
        universe[x][y][z] = universe[x][y][z] ^ 1
